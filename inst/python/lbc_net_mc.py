...
from lbc_helpers_mc import *

def run_lbc_net(data_df, Z_columns, T_column, Y_column, ck, h, 
    ...
    dict
        ...
        - `"point_estimate"`: Point estimate of the ATE or ATT.
        - `"variance"`: Variance of the ATE or ATT estimate.
    ...
    Y_numpy = data_df.loc[:, Y_column].to_numpy(dtype="float32")  # Explicit NumPy conversion
    ...
    # Train LBC-Net Model
    ps_model = lbc_net(p, hidden_dim, L, epsilon, dropout_rate=0.2).to(device)
    ...
    with torch.no_grad():
        ...
        ps_mean, effect_mean, effect_var = mc_dropout_inference_ate_att(
        ps_model, 
        Z_norm, 
        Y=Y_numpy,  
        Tr=T_numpy, 
        ...
        final_LSD_max, final_LSD_mean = lsd_cal(torch.tensor(ps_mean).to(device), T, Z, ck, h, kernel_id, ate)

    print("✅ LBC-Net training completed successfully.")

    # Return Results
    return {
    "propensity_scores": ps_mean.tolist(),     
    "point_estimate": effect_mean,     
    "variance": effect_var,        
